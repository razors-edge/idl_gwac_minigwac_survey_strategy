pro minigwac_observing_area_living_data_template,inputfile,outputfile
close,/all
...
case num_mnt of
1: data_tmp = make_array(4,maxrec)
2: data_tmp = make_array(7,maxrec)
3: data_tmp = make_array(10,maxrec)
4: data_tmp = make_array(13,maxrec)
5: data_tmp = make_array(16,maxrec)
6: data_tmp = make_array(19,maxrec)
endcase


openr,50,inputfile
point_lun,50,0
obs_mode = make_array(maxrec,/long)
for i = 0, maxrec-1 do begin  
readf,50,str
time = TEN((STRSPLIT(str,/EXTRACT))[0])
obs_mode[i] = fix((STRSPLIT(str,/EXTRACT))[3])
;print,'obs_mode',obs_mode[i],fix((STRSPLIT(str,/EXTRACT))[3])
data_tmp[0,i] = time
for j = 1, num_col-1 do begin
    data_tmp[j,i] = (STRSPLIT(str,/EXTRACT))[j]
endfor

goodindex = Where( obs_mode eq 1, goodcount)
IF goodcount GT 0 THEN begin
  case num_mnt of
    1: data = make_array(4,goodcount)
    2: data = make_array(7,goodcount)
    3: data = make_array(10,goodcount)
    4: data = make_array(13,goodcount)
    5: data = make_array(16,goodcount)
    6: data = make_array(19,goodcount)
endcase
  for efs = 0, goodcount-1 do begin
    data[*,efs] = data_tmp[*,goodindex[efs]]
   endfor
endif
endfor

coor = make_array(num_mnt,goodcount,/long)
coor1 = make_array(num_mnt,goodcount,/long)
for efe = 0, goodcount-1 do begin
for k = 0, num_mnt-1 do begin
coor[k,efe] = long(long(data[k*3+1,efe])*100+long(data[k*3+2,efe]))
endfor
endfor
close,50 
